...
class GazeRes18(nn.Module):
    ...
    def forward(self, x_in):
        ...
        return {"pred": angular_output, "gaze": x_in["gaze"], "feature": base_out}

class GazeLmkRes18(nn.Module):
    def __init__(self, drop_p=0.5):
        super(GazeLmkRes18, self).__init__()
        self.img_feature_dim = 256  # the dimension of the CNN feature to represent each frame
        ...
        self.lmk_fc = nn.Sequential(
            nn.Linear(512 * self.base_model.block.expansion, 256),
    ...
    def forward(self, x_in):
        base_out, feature = self.base_model(x_in["face"])
        base_out = torch.flatten(base_out, start_dim=1)
        output = self.drop(base_out)
        output = self.last_layer(output)
        angular_output = output[:, :2]
        pred_landmarks = self.lmk_fc(feature)
        landmarks = x_in["landmark"].flatten(start_dim = 1)
        loss = self.loss(pred_landmarks, landmarks)
        return {"pred": angular_output, "gaze": x_in["gaze"], "feature": feature, "lmk_loss": loss}
